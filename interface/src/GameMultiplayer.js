...
const GameMultiplayer = ({playerName, roomNumber}) => {
    ...
    let {data: intrebari, loading: loading2, error: error2} = useFetch("http://localhost:8080/multiplayer/questions/" + roomNumber);
...
useEffect(() => {
    ...
    turnSocketRef.current.addEventListener("message", async e => {
        ...
        if(turnInfo.update == "yes"){
            intrebari = await fetch("http://localhost:8080/multiplayer/questions/" + roomNumber).then(data => data.json());
        ...
        if(turnInfo.stop != undefined){ 
            fetch("http://localhost:8080/multiplayer/score/" + roomNumber + "/" + playerName).then(data => data.json()).then(data => setFinished(data.points));
    ...
    async function changeTurnFunc(){
        if(changeTurn == true){
            if (turnSocketRef.current.readyState == WebSocket.OPEN) {
                ...
                setChangeTurn(false);
                let finalPoints = await fetch("http://localhost:8080/multiplayer/score/" + roomNumber + "/" + playerName).then(data => data.json());
